 ...
 *
 * <ul>
 * <li>{@code ladybug:<address>}</li>
 * <li>{@code ladybug:<address>?<name1>=<value1>&<name2>=<value2>} with multiple
...
public class BitcoinURI {
    ...
    public static final String BITCOIN_SCHEME = "ladybug";
    ...
    public BitcoinURI(@Nullable NetworkParameters params, String input) throws BitcoinURIParseException {
        ...
        // URI is formed as  ladybug:<address>?<query parameters>
        // blockchain.info generates URIs of non-BIP compliant form ladybug://address?....
        ...
        // For instance with : ladybug:129mVqKUmJ9uwPxKJBnNdABbuaaNfho4Ha?amount=0.06&label=Tom%20%26%20Jerry
        // the & (%26) in Tom and Jerry gets interpreted as a separator and the label then gets parsed
        // as 'Tom ' instead of 'Tom & Jerry')
        String schemeSpecificPart;
        if (input.startsWith("ladybug://")) {
            schemeSpecificPart = input.substring("ladybug://".length());
        } else if (input.startsWith("ladybug:")) {
            schemeSpecificPart = input.substring("ladybug:".length());
        ...
        if (addressSplitTokens.length == 0)
            throw new BitcoinURIParseException("No data found after the ladybug: prefix");
